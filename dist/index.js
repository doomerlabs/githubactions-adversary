...
var GHA_RULE_IDS = {
  ...
  "secret-scope-job": null
  // depot-only P0 mapping
};

// src/job-timeouts.ts
var LONG_RUNNING_SIGNAL = /\b(?:aarch64|arm(?:v\d+)?|buildx|cross[-_ ]?(?:compile|compilation)|e2e|end[-_ ]?to[-_ ]?end|integration|multi[-_ ]?arch|package|packaging|publish|qemu|release)\b/i;
var GITHUB_HOSTED_RUNNER = /^(?:ubuntu|windows|macos)-[a-z0-9_.-]+$/i;
function detectMissingLongRunningJobTimeouts(file, source) {
  const lines = source.split(/\r?\n/);
  const hits = [];
  for (const job of collectJobs(lines)) {
    const runner = readRunner(job.lines);
    if (runner === void 0 || !isCustomRunner(runner.labels, runner.expression)) continue;
    if (hasJobTimeout(job.lines)) continue;
    const searchable = [job.id, ...job.lines.filter((line) => !/^\s*runs-on\s*:/.test(line.trimStart()))].join("\n");
    if (!LONG_RUNNING_SIGNAL.test(searchable)) continue;
    hits.push({
      file,
      line: job.line,
      snippet: `${job.id}:`,
...
function collectJobs(lines) {
  ...
  let current;
  const flush = () => {
    if (current !== void 0) jobs.push(current);
    current = void 0;
  };
...
function readRunner(lines) {
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (value.includes("${{")) return { labels: [], display: value, expression: true };
    if (value.startsWith("[") && value.endsWith("]")) {
      const labels2 = value.slice(1, -1).split(",").map(cleanLabel).filter(Boolean);
      return { labels: labels2, display: `[${labels2.join(", ")}]`, expression: false };
    }
    if (value !== "") {
...
function isCustomRunner(labels, expression) {
  if (expression || labels.length === 0) return false;
  if (labels.some((label) => label.toLowerCase() === "self-hosted")) return true;
...
function evaluateBoolean(expression, stringValues) {
  ...
  const text2 = expression.slice(range.start, range.end).trim();
  const comparison = splitComparison(text2);
  ...
  if (text2.startsWith("!")) {
    const value = evaluateBoolean(text2.slice(1), stringValues);
    return value === void 0 ? void 0 : !value;
  }
  const constant = parseConstant(text2, stringValues);
...
function evaluateConstant(expression, stringValues) {
  ...
  return parseConstant(text2, stringValues);
}
...
function parseConstant(text2, stringValues) {
...
var MAX_OBS = 6;
var GITHUBACTIONS_MODEL_PROMPT = `You are reviewing GitHub Actions workflows for security, supply-chain, and reliability defects.

Authority: unpinned actions, overly broad GITHUB_TOKEN permissions, pull_request_target executing untrusted code, secret exfiltration patterns, missing timeouts on sensitive jobs.
...
var GITHUBACTIONS_MODEL_SCHEMA = {
  type: "object",
  additionalProperties: false,
    ...
    observations: {
      ...
      items: {
        ...
        properties: {
          ...
          category: { type: "string", enum: ["unpinned-action", "permissions", "pull-request-target", "secrets", "supply-chain", "completeness"] },
...
` : source.content;
  ...
  return {
    ...
    request: {
      prompt: GITHUBACTIONS_MODEL_PROMPT,
      input,
      schema: GITHUBACTIONS_MODEL_SCHEMA,
      budget: { maximumOutputTokens: 4096, timeoutMs: 12e4 }
    }
  };
}
async function runModelGithubActionsReview(ctx, detections, sources, staticSeverities = [], staticPrimaryConcern) {
  ...
  try {
    ...
    await applyModelGithubActionsReview(ctx, result.output, evidenceById, staticSeverities, staticPrimaryConcern);
    return "applied";
  } catch (error) {
...
async function applyModelGithubActionsReview(ctx, output, evidenceById, staticSeverities, staticPrimaryConcern) {
  ...
  for (const observation of output.observations.slice(0, MAX_OBS)) {
    ...
    ctx.review.observe({
      key: `githubactions.model.${observation.id}`,
...
var byId2 = new Map(spec.rules.map((rule) => [rule.id, rule]));
async function analyzeRepository(ctx) {
  ...
  for (const file of sources) {
    ...
    const timeoutRule = byId2.get("gha.custom-runner.missing-timeout");
    if (timeoutRule !== void 0) {
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "github-actions", version: "0.0.21", review: { maximumFindings: 8 } });
  registerRules(app);
